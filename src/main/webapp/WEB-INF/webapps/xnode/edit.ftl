...
<body [@flash_message /]>
	<div class="path">
		<a href="/xnode/common/index.jhtml">${message("xnode.path.index")}</a>&raquo;${message("xnode.node.edit")}
	</div>
	<form id="editForm" action="upadte.jhtml" method="post">
	
		<div class="title title-box move-left">
			<label>标题</label>
			<textarea class="text-area text" name="title" id="title">${nodeBean.title}</textarea>
		</div>
		
		<div class="content">
			<label>内容</label>
			<div class="text-box">
				<textarea class="text-area text" name="content" id="content">${nodeBean.content}</textarea>
			</div>
		</div>
		<div>
